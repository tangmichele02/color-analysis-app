...
export default function ColorAnalysisApp() {
  ...
  const goToSlide = (index: number) => {
    setCurrentIndex(Math.max(0, Math.min(index, palettes.length - 1)));
  };
  ...
  const handleDragEnd = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (Math.abs(diff) > 50) {
      if (diff > 0 && currentIndex < palettes.length - 1) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#faf8f5]">
      ...
      <main className="flex-1 flex flex-col px-4 md:px-8 py-8 max-w-[1600px] mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-center max-w-[1400px] w-full">
            ...
            <div className="hidden md:flex flex-col gap-4 min-w-[140px]">
              <div className="font-display text-sm font-bold text-center text-[#2d2420] mb-2 uppercase tracking-widest">
                {palettes[currentIndex].name}
              </div>
              {palettes[currentIndex].colors.slice(0, 4).map((color, index) => (
                <div 
                  key={index}
            ...
            >
              ...
              <div className={`absolute inset-0 pointer-events-none transition-opacity duration-400 ${cameraActive ? 'opacity-100' : 'opacity-0'}`}>
                <div 
                  ...
                  style={{ backgroundColor: palettes[currentIndex].dominantColor }}
                />
              </div>
            ...
            <div className="hidden md:flex flex-col gap-4 min-w-[140px]">
              <div className="font-display text-sm font-bold text-center text-[#2d2420] mb-2 uppercase tracking-widest">
                {palettes[currentIndex].description.split('—')[0]}
              </div>
              {palettes[currentIndex].colors.slice(4, 8).map((color, index) => (
                <div 
                  key={index}
                  className="group w-full aspect-square rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 cursor-grab active:cursor-grabbing hover:scale-110 relative"
                  style={{ 
        ...
        {/* Instructions */}
        <div className="text-center mt-12 px-6 md:px-8 py-8 bg-white rounded-3xl border border-[#e8e3dc] max-w-[600px] mx-auto animate-fade-in-up-delay-2">
          <h3 className="font-display text-2xl mb-4 text-[#2d2420]">
            How It Works
          </h3>
          <p className="text-[#6b5d54] leading-relaxed">
            Start your camera and swipe through the seasonal color palettes to see which harmonizes 
            best with your natural coloring. <strong>Drag individual colors onto your video</strong> to 
            see them up close to your face. Each season has its own unique character—Spring's warmth 
            and clarity, Summer's soft coolness, Autumn's rich depth, or Winter's striking contrast. 
            Click on dragged colors to remove them. Trust your instincts and notice which palette makes you feel most radiant.
          </p>
        </div>
      </main>
    </div>
